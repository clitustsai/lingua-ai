  ...
  const generate = async (t?: string) => {
    ...
    try {
      ...
      if (data.error || (!data.questions && !data.vocabulary)) {
        setError("AI không t?o du?c bài h?c. Th? l?i nhé!");
        return;
      }
      if (!isPremium) incrementUsage("generateLesson");
      setLesson(data);
    } catch {
      setError("L?i k?t n?i. Vui lòng th? l?i!");
    ...
    <div className="p-5 max-w-2xl">
      <div className="pt-2 mb-5">
        <h1 className="text-xl font-bold text-white flex items-center gap-2">
          <Sparkles className="w-5 h-5 text-yellow-400" /> T?o bài h?c t? d?ng
        </h1>
        <p className="text-sm text-gray-400 mt-1">Nh?p ch? d? ? AI t?o câu h?i, sample answer, vocab ngay</p>
      </div>
      ...
      <div className="rounded-2xl p-4 mb-4" style={{ background: "rgba(26,16,53,0.8)", border: "1px solid rgba(139,92,246,0.2)" }}>
        <input
          ...
          placeholder='Ví d?: "IELTS Speaking Part 1", "Grammar: Past Perfect"...'
        ...
        <button onClick={() => generate()} disabled={loading || !topic.trim()}
          ...
          {loading ? <><Loader2 className="w-4 h-4 animate-spin" /> Ðang t?o bài h?c...</> : <><Sparkles className="w-4 h-4" /> T?o bài h?c</>}
        </button>
        {!isPremium && (
          <p className="text-center text-xs text-gray-600 mt-2">
            Còn {getRemainingUses("generateLesson", isPremium)}/{FREE_LIMITS.generateLesson} l?n hôm nay
      ...
      {loading && (
        <div className="flex flex-col items-center py-10 gap-3">
          ...
          <p className="text-gray-500 text-sm">AI dang t?o bài h?c cho "{topic}"...</p>
        </div>
      )}
        ...
        <div className="flex flex-col gap-4 animate-fade-in-up">
          ...
          {lesson.questions?.length > 0 && (
            <div className="rounded-2xl overflow-hidden" style={{ background: "rgba(26,16,53,0.8)", border: "1px solid rgba(139,92,246,0.15)" }}>
              <button onClick={() => toggle("questions")}
                ...
                <span className="flex items-center gap-2"><Mic className="w-4 h-4 text-pink-400" /> Câu h?i luy?n t?p ({lesson.questions.length})</span>
          ...
          {lesson.grammarTips?.length > 0 && (
            <div className="rounded-2xl p-4" style={{ background: "rgba(234,179,8,0.08)", border: "1px solid rgba(234,179,8,0.2)" }}>
              <p className="text-xs text-yellow-400 font-semibold uppercase tracking-wide mb-3">?? Ng? pháp c?n nh?</p>
