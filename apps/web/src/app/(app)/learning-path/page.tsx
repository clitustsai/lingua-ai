...
const GOALS = [
  ...
  { id: "business", label: "Công vi?c", emoji: "??", desc: "Ti?ng Anh van phòng" },
  { id: "daily", label: "Giao ti?p hàng ngày", emoji: "??", desc: "Nói chuy?n t? nhiên" },
  { id: "culture", label: "Van hóa & Gi?i trí", emoji: "??", desc: "Xem phim, nghe nh?c" },
  { id: "academic", label: "H?c thu?t", emoji: "??", desc: "Ð?c tài li?u, nghiên c?u" },
];
...
export default function LearningPathPage() {
  ...
  const startRecording = () => {
    ...
    if (!SR) { alert("Dùng Chrome d? nh?n di?n gi?ng nói."); return; }
    const rec = new SR();
    ...
    rec.onresult = async (e: any) => {
      ...
      const feedback = score >= 80 ? "Phát âm t?t l?m! ??"
        : score >= 60 ? "Khá t?t, ti?p t?c luy?n t?p! ??"
        : "Th? l?i, nói rõ hon nhé! ??";
  ...
  const generate = async () => {
    ...
    try {
      ...
      setStep("onboard");
      alert("Không th? t?o l? trình. Th? l?i nhé!");
    }
  };
  ...
  const openDay = async (idx: number) => {
    ...
    try {
      ...
      if (data.error || (!data.vocabulary && !data.grammarPoint && !data.quiz)) {
        setDayLesson({ _error: data.error || "Không th? t?i bài h?c. Th? l?i nhé!" });
      } else {
        setDayLesson(data);
      }
    } catch {
      setDayLesson({ _error: "L?i k?t n?i. Vui lòng th? l?i!" });
    } finally {
      setLoadingDay(false);
    }
  };
    ...
    <div className="p-5 max-w-lg">
      <div className="mb-6 pt-4">
        <h1 className="text-2xl font-bold text-white flex items-center gap-2">
          <Sparkles className="w-6 h-6 text-yellow-400" /> L? trình h?c cá nhân
        </h1>
        <p className="text-sm text-gray-400 mt-1">AI t?o k? ho?ch h?c riêng cho b?n</p>
      </div>

      <div className="flex flex-col gap-5">
        <div>
          <p className="text-sm font-semibold text-gray-300 mb-3">?? M?c tiêu c?a b?n</p>
        ...
        <div>
          <p className="text-sm font-semibold text-gray-300 mb-3">?? Trình d? hi?n t?i</p>
          ...
          {LEVELS.some(l => !canAccessLevel(l)) && (
            <p className="text-xs text-yellow-400/70 mt-2">?? Thi d?t trình d? hi?n t?i d? m? khóa trình d? ti?p theo</p>
          )}
        </div>

        <div>
          <p className="text-sm font-semibold text-gray-300 mb-3">?? H?c m?y ngày/tu?n?</p>
        ...
        <div className="rounded-xl p-4 flex items-center gap-3" style={{ background: "rgba(139,92,246,0.1)", border: "1px solid rgba(139,92,246,0.2)" }}>
          ...
          <div>
            ...
            <p className="text-xs text-gray-400">M?c tiêu: {GOALS.find(g => g.id === selectedGoal)?.label} · {selectedLevel} · {daysPerWeek} ngày/tu?n</p>
          </div>
        </div>
        ...
        <button onClick={generate}
          ...
          <Sparkles className="w-5 h-5" /> T?o l? trình c?a tôi
        </button>
        {!isPremium && (
          <p className="text-center text-xs text-gray-600">
            Còn {getRemainingUses("lesson", isPremium)}/{FREE_LIMITS.lesson} l?n hôm nay
          </p>
        )}
      </div>
    </div>
  ...
  if (step === "generating") return (
    <div className="flex flex-col items-center justify-center h-screen gap-4">
      ...
      <p className="text-white font-semibold">AI dang t?o l? trình cho b?n...</p>
      <p className="text-gray-400 text-sm">Ch? m?t vài giây</p>
    </div>
  );
      ...
      <div className="p-5 max-w-lg">
        ...
        {loadingDay ? (
          <div className="flex flex-col items-center justify-center h-64 gap-3">
            ...
            <p className="text-gray-400 text-sm">Ðang t?o bài h?c hôm nay...</p>
          </div>
        ) : dayLesson?._error ? (
          ...
          <div className="flex flex-col gap-5">
            ...
            {dayLesson.vocabulary?.length > 0 && (
              <div className="rounded-2xl overflow-hidden" style={{ background: "rgba(26,16,53,0.8)", border: "1px solid rgba(139,92,246,0.15)" }}>
                <div className="px-4 py-3 border-b border-white/5 flex items-center gap-2">
                  ...
                  <p className="text-sm font-semibold text-white">T? v?ng hôm nay</p>
                </div>
            ...
            {dayLesson.speakingPrompt && (
              <div className="rounded-2xl p-4" style={{ background: "rgba(26,16,53,0.8)", border: "1px solid rgba(236,72,153,0.2)" }}>
                <p className="text-pink-400 font-semibold text-sm flex items-center gap-2 mb-2">
                  <Mic className="w-4 h-4" /> Luy?n nói
                </p>
                ...
                {!speakingDone ? (
                  <div className="flex gap-2">
                    <button
                      ...
                      {isRecording
                        ? <><MicOff className="w-4 h-4" /> D?ng ghi âm</>
                        : <><Mic className="w-4 h-4" /> B?t d?u nói</>}
                    </button>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-green-400 text-sm font-semibold">Ðã hoàn thành luy?n nói!</span>
            ...
            {dayLesson.dailyChallenge && (
              <div className="rounded-2xl p-4 flex items-start gap-3" style={{ background: "rgba(251,191,36,0.1)", border: "1px solid rgba(251,191,36,0.2)" }}>
                ...
                <div>
                  <p className="text-yellow-400 font-semibold text-sm mb-1">Th? thách hôm nay</p>
              ...
              <div>
                {dayLesson.speakingPrompt && !speakingDone && (
                  <p className="text-center text-xs text-pink-400 mb-3">
                    ?? Hoàn thành ph?n luy?n nói d? ti?p t?c
                  </p>
                )}
                <button
                  onClick={completeDay}
        ...
        {showExitConfirm && (
          <div className="fixed inset-0 z-50 flex items-end justify-center pb-8 px-4"
            ...
            <div className="w-full max-w-sm rounded-3xl p-6 text-center"
              ...
              <p className="text-white font-bold text-lg mb-5">B?n có ch?c ch?n mu?n thoát không?</p>
  ...
  return (
    <div className="p-5 max-w-lg">
        ...
        <div className="px-4 py-3 flex flex-col gap-3">
          ...
          {!toeicChecked ? (
            <button onClick={() => setToeicChecked(true)} disabled={Object.keys(toeicAnswers).filter(k => k.startsWith(toeicPart)).length < toeicQs.length}
              ...
              Ki?m tra ({Object.keys(toeicAnswers).filter(k => k.startsWith(toeicPart)).length}/{toeicQs.length} câu)
            </button>
          ) : (
            <div className="flex items-center justify-between px-4 py-3 rounded-xl" style={{ background: "rgba(59,130,246,0.1)", border: "1px solid rgba(59,130,246,0.2)" }}>
              <span className="text-white text-sm font-bold">{toeicScore}/{toeicQs.length} dúng</span>
              <button onClick={() => { setToeicAnswers({}); setToeicChecked(false); }} className="text-xs text-gray-400 hover:text-white">Làm l?i</button>
            </div>
          )}
        </div>
      </div>
      ...
      <div className="flex flex-col gap-2 mb-5">
        <p className="text-xs text-gray-500 font-semibold uppercase tracking-wide mb-1">K? ho?ch 7 ngày</p>
        {(learningPath.days ?? []).map((day: any, i: number) => {
          const done = pathDaysDone.includes(i + 1);
          const isToday = i === doneDays;
          const prevDone = i === 0 || pathDaysDone.includes(i); // ngày tru?c dã hoàn thành
          ...
          return (
            <button key={i} onClick={() => isLocked ? router.push("/premium") : (!isSequenceLocked && openDay(i))}
              ...
              {isSequenceLocked && (
                <span className="text-xs text-gray-600 shrink-0">Hoàn thành ngày tru?c</span>
              )}
