...
const SKILL_MODES = [
  ...
  { id: "grammar",    label: "Ng? pháp",  emoji: "??", color: "#3b82f6", desc: "T?p trung ng? pháp" },
  { id: "vocabulary", label: "T? v?ng",   emoji: "??", color: "#10b981", desc: "M? r?ng v?n t?" },
  { id: "writing",    label: "Vi?t",      emoji: "??", color: "#f59e0b", desc: "Luy?n vi?t câu" },
...
const LESSON_TOPICS = [
  { emoji: "??", label: "Chào h?i",        topic: "greetings and introductions" },
  ...
  { emoji: "??", label: "Ði?n tho?i",       topic: "phone conversation skills" },
  { emoji: "??", label: "Nhà ?",            topic: "housing and renting vocabulary" },
  { emoji: "??", label: "Tài chính",        topic: "banking and financial vocabulary" },
  { emoji: "??", label: "H?c thu?t",        topic: "academic English for school" },
  { emoji: "??", label: "Công ngh?",        topic: "technology and IT vocabulary" },
  { emoji: "??", label: "Ðàm phán",         topic: "negotiation and persuasion phrases" },
  { emoji: "??", label: "C?m xúc",          topic: "expressing emotions and feelings" },
  { emoji: "??", label: "Van hóa",          topic: "culture and social conversation" },
  { emoji: "??", label: "Th?i s?",          topic: "news and current events vocabulary" },
  { emoji: "??", label: "Gi?i trí",         topic: "hobbies and entertainment" },
  { emoji: "????????", label: "Gia dình",    topic: "family and relationships" },
  { emoji: "??", label: "Giao thông",       topic: "transportation and directions" },
  { emoji: "??", label: "Email van phòng",  topic: "professional email writing" },
  { emoji: "??", label: "Thuy?t trình",     topic: "presentations and public speaking" },
];
...
export default function HomeworkPage() {
  ...
  const generate = async () => {
    if (!isPremium && !canUseFeature("homework", isPremium)) {
      setError(`Ðã dùng h?t ${FREE_LIMITS.homework} l?n/ngày. Nâng c?p VIP d? dùng không gi?i h?n!`);
      return;
    }
    ...
    try {
      ...
      if (!data.exercises?.length) {
        setError("AI không t?o du?c bài t?p. Th? l?i nhé!");
        return;
      }
      setHomework(data);
      ...
      const errText = msg.includes("429") ? "AI dang b?n, th? l?i sau 1 phút." : "L?i k?t n?i. Vui lòng th? l?i!";
      setError(errText);
      setAiError(errText);
    ...
    <div className="p-5 max-w-xl">
      ...
      <div className="pt-2 mb-5 flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">Bài t?p cá nhân hóa · AI ch?m di?m · Theo dõi ti?n d?</p>
        </div>
      ...
      {!homework && !loading && (
        <div className="flex flex-col gap-5">
          ...
          {/* Difficulty */}
          <div>
            <p className="text-xs text-gray-500 font-semibold uppercase tracking-wide mb-3">Ð? khó</p>
          ...
          {/* Topic selector */}
          <div>
            <p className="text-xs text-gray-500 font-semibold uppercase tracking-wide mb-3">Ch? d? bài h?c <span className="text-gray-700 normal-case font-normal">(tu? ch?n)</span></p>
          ...
          <button onClick={generate}
            ...
            <GraduationCap className="w-5 h-5" /> Nh?n bài t?p hôm nay
          </button>
          {!isPremium && (
            <p className="text-center text-xs text-gray-600 mt-1">
              Còn {getRemainingUses("homework", isPremium)}/{FREE_LIMITS.homework} l?n hôm nay
              {getRemainingUses("homework", isPremium) === 0 && (
                <button onClick={() => router.push("/premium")} className="ml-1 text-yellow-500 underline">Nâng c?p VIP</button>
              )}
            </p>
          )}
        </div>
      ...
      {loading && (
        <div className="flex flex-col items-center justify-center h-64 gap-4">
          ...
          <p className="text-gray-400 text-sm">AI dang t?o bài t?p cho b?n...</p>
        </div>
      )}
        ...
        <div className="flex flex-col gap-4">
          ...
          {homework.exercises?.map((ex: Exercise, i: number) => (
            <div key={ex.id} className="rounded-2xl p-4 animate-fade-in-up"
              ...
              <button onClick={() => speakText(ex.question, settings.targetLanguage.code, settings.speechRate)}
                ...
                <Volume2 className="w-3.5 h-3.5" /> Nghe câu h?i
              </button>
              ...
              {ex.hint && (
                <button onClick={() => setShowHints(p => ({ ...p, [ex.id]: !p[ex.id] }))}
                  ...
                  {showHints[ex.id] ? "?n g?i ý" : "?? Xem g?i ý"}
                </button>
              )}
          ...
          <div className="rounded-2xl overflow-hidden" style={{ background: "rgba(10,6,24,0.95)", border: "1px solid rgba(59,130,246,0.2)" }}>
            <div className="px-4 pt-3 pb-2 border-b border-white/5 flex items-center justify-between">
              <p className="text-white font-bold text-sm">?? Luy?n t?p TOEIC — 200 câu</p>
          ...
          <button onClick={grade} disabled={!allAnswered || grading}
            ...
            {grading
              ...
              : <><Star className="w-5 h-5" /> N?p bài · {fmt(elapsed)}</>
            }
          </button>
        </div>
      )}
        ...
        <div className="flex flex-col gap-4 animate-fade-in-up">
          <div className="rounded-2xl p-5 text-center"
            ...
            <div className="flex items-center justify-center gap-4 mt-2 text-sm">
              ...
              <span className="text-gray-500">{gradeResult.results?.filter((r: GradeResult) => r.correct).length}/{gradeResult.results?.length} dúng</span>
            </div>
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1">
                    ...
                    {!r.correct && r.correction && (
                      <p className="text-sm text-green-300 font-medium mb-1">Ðáp án: {r.correction}</p>
                    )}
