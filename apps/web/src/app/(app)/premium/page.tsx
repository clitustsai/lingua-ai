...
export default function PremiumPage() {
  ...
  const notifyPayment = async () => {
    ...
    setNotifying(true);
    try {
      await fetch("/api/payment/notify", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ userId: user.id, userEmail: user.email, plan, addInfo: qrData["addInfo"], amount: qrData["amount"] }) });
      setNotified(true);
    } catch { alert("Loi gui."); } finally { setNotifying(false); }
  };
  ...
  return (
    <div className="max-w-lg pb-10" style={{ background: "#0f0a1e", minHeight: "100vh" }}>
      <div className="px-5 pt-10 pb-8 text-center" style={{ background: "linear-gradient(135deg,#1a0533,#0f0a1e)" }}>
      ...
      <div className="px-5 space-y-5">
        <div className="flex gap-2 p-1 rounded-2xl" style={{ background: "rgba(15,10,30,0.8)", border: "1px solid rgba(255,255,255,0.06)" }}>
          {PLANS.map((p) => (
            <button key={p.id} onClick={() => { setPlan(p.id as "monthly" | "yearly"); setQrData(null); setNotified(false); }}
                ...
                <div key={item.key} className="flex justify-between items-center text-xs">
                  ...
                  <div className="flex items-center gap-1">
                    ...
                    {(item.key === "acc" || item.key === "info") && (
                      <button onClick={() => copyText(item.value, item.key)} className="text-gray-500 hover:text-white ml-1">
