...
function PayModal({ item, onClose }: { item: PayItem; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-4"
      ...
      <div className="w-full max-w-sm rounded-3xl overflow-hidden animate-fade-in-scale"
        ...
        <div className="p-4 flex flex-col gap-4">
          <div className="grid grid-cols-3 gap-2">
            {[
              { src: "/qr-momo.png", label: "MoMo", color: "#ae00ff" },
              { src: "/qr-zalopay.png", label: "ZaloPay", color: "#0068ff" },
              { src: "/qr-acb.png", label: "ACB", color: "#0066cc" },
            ].map(q => (
        ...
        <div className="flex flex-col gap-4">
          {COURSES.map(course => (
            <div key={course.id} className="rounded-2xl overflow-hidden" style={{ background: "rgba(26,16,53,0.8)", border: "1px solid rgba(139,92,246,0.15)" }}>
              <div className="relative aspect-video bg-gray-900 cursor-pointer group" onClick={() => setSelectedCourse(course)}>
        ...
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-4"
          ...
          <div className="w-full max-w-lg rounded-3xl overflow-hidden animate-fade-in-scale"
            ...
            <div className="aspect-video relative">
              ...
              <button onClick={() => setSelectedCourse(null)} className="absolute top-3 right-3 w-8 h-8 rounded-full bg-black/60 flex items-center justify-center text-white hover:bg-black/80 transition-colors">
                <X className="w-4 h-4" />
              </button>
            </div>
            <div className="p-5">
              ...
              <div className="flex gap-3">
                ...
                <button onClick={() => setSelectedCourse(null)} className="px-4 py-3 rounded-xl border border-gray-700 text-gray-400 hover:text-white text-sm transition-colors">Dong</button>
              </div>
            </div>
          </div>
        </div>
